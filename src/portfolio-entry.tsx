...
export function PortfolioDataIsland() {
  ...
  return <div className="portfolio-data-shell"><div className="filter-row" role="group" aria-label="Filter karya">{(categories.data ?? []).map((item) => <button key={item.label} className={`filter ${category === item.value ? 'active' : !category && !item.value ? 'active' : ''}`} aria-pressed={category === item.value || (!category && !item.value)} onClick={() => setCategory(item.value)}>{item.label}{item.value ? null : <span>{query.data?.length ?? '—'}</span>}</button>)}<label className="portfolio-search"><span className="sr-only">Cari portfolio</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Cari karya…" /></label></div>{(query.isLoading || categories.isLoading) && <p className="data-status">Memuat karya dari Supabase…</p>}{(query.isError || categories.isError) && <p className="data-status data-error">Portfolio belum dapat dimuat. Pastikan env Supabase dan policy public read sudah dikonfigurasi.</p>}{query.data && query.data.length === 0 && <p className="data-status">Belum ada portfolio yang cocok.</p>}{query.data && query.data.length > 0 && <div className="work-grid">{query.data.map((item) => <PortfolioCard key={item.id} item={item} />)}</div>}</div>;
}
