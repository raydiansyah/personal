...
export async function getSlideBySlug(slug: string, accessCode = ''): Promise<{ slide: PublicSlide; slides: PublicSlideLink[] } | null> { const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/verify-public-slide`, { method: 'POST', headers: { apikey: import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY || import.meta.env.VITE_SUPABASE_ANON_KEY || '', 'Content-Type': 'application/json' }, body: JSON.stringify({ slug, access_code: accessCode || undefined }) }); let payload: { error?: string; slide?: PublicSlide; slides?: PublicSlideLink[] }; try { payload = await response.json() as typeof payload; } catch { throw new SlideAccessError('slide_access_failed'); } if (response.status === 404) return null; if (!response.ok || !payload.slide || !payload.slides) throw new SlideAccessError((payload.error as SlideAccessError['code']) || 'slide_access_failed'); return { slide: payload.slide, slides: payload.slides }; }
