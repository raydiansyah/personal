 ...
 * Dependencies: Cloudflare Turnstile browser API; VITE_TURNSTILE_SITE_KEY
...
export function TurnstileWidget({ onToken, onError }: { onToken: (token: string) => void; onError?: (message: string) => void }) {
  ...
  useEffect(() => {
    ...
    if (!siteKey) {
      onError?.('CAPTCHA belum dikonfigurasi.');
      return;
    }
    let cancelled = false;
    void loadTurnstileScript().then(() => {
      ...
      widgetIdRef.current = window.turnstile.render(containerRef.current, {
        ...
        'error-callback': () => onError?.('CAPTCHA tidak dapat diverifikasi. Coba lagi.'),
      });
    }).catch((error: unknown) => onError?.(error instanceof Error ? error.message : 'CAPTCHA tidak dapat dimuat.'));
    return () => {
      cancelled = true;
  ...
  return <div ref={containerRef} aria-label="Verifikasi CAPTCHA" />;
}
